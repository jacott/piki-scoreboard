(function (test, v) {
  buster.testCase('models/server/change-log:', {
    setUp: function () {
      test = this;
      v = {};
    },
    ...
    "registerObserveModel": {
      setUp: function () {
        TH.stubOplog();
        v.Foo = {modelName: 'Foo'};
        AppModel.ChangeLog.registerObserveModel(v.Foo);
      },
    ...
    "test no saveRpc": function () {
      refute(TH.getMethodHandlers()['ChangeLog.save']);
    },
    ...
    "test parentId": function () {
      ...
      var cl = AppModel.ChangeLog.findOne({model_id: result._id});
    ...
    "test no changes": function () {
      ...
      assert.difference(0, AppModel.ChangeLog, function () {
    ...
    "test save on update": function () {
      ...
      assert.same(AppModel.ChangeLog.find({model: 'Climber'}).count(), 1);

      var cl = AppModel.ChangeLog.findOne();
      ...
      assert.same(AppModel.ChangeLog.find({model: 'Climber'}).count(), 1);
      refute(AppModel.ChangeLog.exists(cl._id));
      cl =  AppModel.ChangeLog.findOne();
    ...
    'test save on create': function () {
      ...
      assert.same(AppModel.ChangeLog.find({}).count(), 1);
  ...
  TH.ChangeLogTest = {
    assertLogChanges: function (test, model, parent) {
      var doc;

      TH.login(function () {
        model = model.modelName || model;
        doc = TH.Factory['build'+model]();
        doc.$save();
      });

      if (parent)
        assert.same(AppModel.ChangeLog.find({model: model, model_id: doc._id, parent: parent.modelName, parent_id: doc.$parentId()}).count(), 1);
      else
        assert.same(AppModel.ChangeLog.find({model: model, model_id: doc._id, parent: model, parent_id: doc._id}).count(), 1);
    },
  };
})();
