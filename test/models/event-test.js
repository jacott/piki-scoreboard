(function (test, v) {
  buster.testCase('models/event:', {
    ...
    "competitor registration": {
      setUp: function () {
        v.cat1 = TH.Factory.createCategory({_id: 'cat1', heatFormat: 'F8QQ'});
        v.cat2 = TH.Factory.createCategory({_id: 'cat2', heatFormat: 'F8F26QQ'});
      ...
      "test new category": function () {
        ...
        result.$$save();
        assert.equals(v.event.$reload().heats, {cat1: 'F8QQ'});

        var result = TH.Factory.buildResult({category_id: v.cat2._id});
        result.$$save();
        assert.equals(v.event.$reload().heats, {cat1: 'F8QQ', cat2: 'F8F26QQ'});
      },
