define((require, exports, module) => {
  ...
  return (ChangeLog) => {
    ...
    const observeChanges = (subject, callback) => {
      subject.afterLocalChange(async (dc) => {
        ...
        if (dc.isDelete) {
          ...
          cl = await ChangeLog.create(params);
        } else {
          cl = await createChangeLog(params, subject, dc);
        }
        callback?.(cl, doc, dc.undo);
      });
    };
    ...
    util.merge(ChangeLog, {
      logChanges(subject, options={}) {
        ...
        subject._parentIdField = options.parent !== undefined &&
          (options.parentIdField || util.uncapitalize(options.parent.modelName) + '_id');
      ...
      async create(attributes) {
        ...
        await ChangeLog.docs.insert(attributes);
