isServer && define((require, exports, module) => {
  ...
  const createResults = async (ans, opts) => {
    for (const id in opts) {
      await Factory.createClimber({name: 'climber_' + id, _id: 'cl' + id.slice(1)});
  ...
  TH.testCase(module, ({before, after, beforeEach, afterEach, group, test}) => {
    ...
    test('summarize speed', async () => {
      const cat = await Factory.createCategory({type: 'S'});
      ...
      await ev.$update('date', '2019-11-02');

      assert.same(await ClimberRanking.query.count(), 0);

      await ev.$update('closed', true);

      const sum = await ClimberRanking.db.query('select * from "ClimberRanking" order by climber_id');
    ...
    test('summarize lead', async () => {
      const cat = await Factory.createCategory({type: 'LQQF26F8'});
      const ev = await Factory.createEvent();
      ...
      await res.r04.$update({scores: [0.4, 1050, 400]});

      await ev.$update('closed', true);

      assert.equals(
        (await ClimberRanking.db.query('select * from "ClimberRanking" order by climber_id'))
