define((require, exports, module) => {
  ...
  class ClimberRanking extends Model.BaseModel {
    static async summarize(event) {
      const {heats, _id: event_id} = event;

      const {db} = ClimberRanking;

      await db.query(`delete from "ClimberRanking" where event_id = $1`, [event_id]);

      for (const category_id in heats) {
        const format = heats[category_id];
        const query = await Result.query.withIndex(Result.eventCatIndex, {event_id, category_id}).fetch();
        switch (format[0]) {
        case 'S': {
          ...
          for (const r of results) {
            ...
            await ClimberRanking.docs.insert({
        ...
        default: {
          ...
          for (let i = 0; i < query.length; ++i, prev = r) {
            r = query[i];
            if (prev === undefined || compareResults(prev, r) !== 0) {
              rank = i + 1;
            }
  ...
  module.onUnload(Event.onChange((dc) => {
    if (dc.isChange && dc.hasField('closed') && dc.doc.closed) {
      return ClimberRanking.summarize(dc.doc);
    }
  }));
