(function (test, v) {
  buster.testCase('packages/app-pages/test/client/markdown-editor-mention:', {
    setUp: function () {
      test = this;
      v = {};
      TH.initMarkdownEditor(v);
      test.spy(Bart, 'stopEvent');
      v.fooFunc = test.stub();
    ...
    "test typing @g": function () {
      assert.dom(v.input, function () {
        assert.same(App.Markdown.fromHtml(v.range.startContainer.parentNode), 'hello ');
    ...
    "test @ followed by -> ->": function () {
      assert.dom(v.input, function () {
        TH.keypress(this, '@', true);
        assert.same(App.Markdown.fromHtml(this), 'hello @');
    ...
    "at list menu": {
      setUp: function () {
        v.fooFunc = function (frag, text) {
          if (text === 'g') {
            frag.appendChild(v.div1 = Bart.html({text: 'Geoff Jacobsen', "data-id": "g1"}));
            frag.appendChild(v.div2 = Bart.html({text: 'Gordon Snow', "data-id": "g2"}));
            frag.appendChild(v.div3 = Bart.html({text: 'Gayle Gunter', "data-id": "g3"}));
          }
          if (text === 'jjg') {
            frag.appendChild(v.div2 = Bart.html({text: 'James J Gooding', "data-id": "j2"}));
            frag.appendChild(v.div3 = Bart.html({text: '(*)', "data-id": "j3", span: {className: 'name', text: 'Josiah<JG>'}}));
          }
        };
      ...
      "test focusout aborts list": function () {
        assert.dom('.mdList', function () {
          ...
          assert.isTrue(Bart.getCtx(this).mousedown);
          TH.trigger(this, 'focusout');
          assert.isNull(Bart.getCtx(this).mousedown);
        });
      ...
      "test tab": function () {
        ...
        assert.dom('.input', function () {
          assert.same(App.Markdown.fromHtml(this), 'hello @[Geoff Jacobsen](g1) ');
        });
      },
      ...
      "test shift tab": function () {
        ...
        assert.dom(v.input, function () {
          assert.same(App.Markdown.fromHtml(this), 'hello @g');

          refute.dom('.ln');
          refute.dom('.lm');

          document.execCommand('insertText', null, 'w');
          assert.same(App.Markdown.fromHtml(this), 'hello w');
        });
      },
    ...
    "keydown": {
      ...
      "test escape pressed": function () {
        ...
        assert.dom(v.input, function () {
          assert.same(App.Markdown.fromHtml(this), 'hello @h');
    ...
    "test arrow right": function () {
      ...
      assert.dom(v.input, function () {
        ...
        document.execCommand('insertText', null, 'w');
        assert.same(App.Markdown.fromHtml(this), 'hello @henryw');
      });
    ...
    "test clear": function () {
      ...
      assert.dom('.mdEditor', function () {
        ...
        Bart.MarkdownEditor.clear(this);
        assert.dom('>.input', '');
    ...
    "test button events don't propagate": function () {
      ...
      assert.dom(v.input, function () {
        this.appendChild(v.button = Bart.html({tag: 'button'}));
        TH.click(v.button);
