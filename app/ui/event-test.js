isClient && define((require, exports, module) => {
  ...
  TH.testCase(module, ({beforeEach, afterEach, group, test}) => {
    ...
    test('event subscribing', () => {
      const events = Factory.createList(2, 'createEvent', (index, options) => {
    ...
    test('showing series tab', () => {
      const series = Factory.createList(2, 'createSeries', (index, options) => {
        options.date = '2014/01/0' + (6 + index);
      });
    ...
    test('adding new event', () => {
      ...
      let tt2 = Factory.createTeamType({default: true});
      Route.gotoPage(sut.Index);
    ...
    group('speed', () => {
      ...
      test('qual rerun', () => {
        const cat = Factory.createCategory({type: 'S'});
        const event = Factory.createEvent({heats: {[cat._id]: 'SC'}});
        const res = Factory.createResult();
      ...
      test('print', () => {
        ...
        const event = Factory.createEvent({heats: {[cat1._id]: 'S4321', [cat2._id]: 'SC'}});
    ...
    group('select', () => {
      beforeEach(() => {
        TH.login();
        v.tt = Factory.createList(2, 'createTeamType');
        v.event = Factory.createEvent({teamType_ids: [v.tt[0]._id]});

        v.cats = Factory.createList(3, 'createCategory', (index, options) => {
          options.shortName = 'YA ' + index;
          options.name = 'Youth A ' + index;
        });
        v.c1 = Factory.buildCompetitor({category_ids: [v.cats[0]._id]});
        v.c2 = Factory.buildCompetitor({category_ids: [v.cats[1]._id]});
        v.c1.$$save();
        v.c2.$$save();

        v.event2 = Factory.createEvent();
      ...
      test('add category', () => {
        ...
        const comp = Factory.buildCompetitor({event_id: v.event._id, category_ids: [v.cats[2]._id]});
        assert.dom('.categories', () => {
      ...
      group('Edit', () => {
        ...
        test('add category', () => {
          const comp = Factory.buildCompetitor({category_ids: [v.cats[2]._id]});
          assert.dom('.categories', () => {
